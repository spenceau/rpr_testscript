...
foreach ($blendFile in $blendFiles) {
    ...
    sudo python cmd_script.py --blender-path "$BlenderExe" --scene-path "$BlendFilesSubdir" --scene-name "$scene"

    # Set RENDER_SUBDIR to the same as SCENE
    #$RenderSubdir = $scene

    # Compare generated render with ground truth/actual
    sudo python compare_render.py --scene "$scene" --ground_truth "$GroundTruthSubdir" --render "$RenderSubdir"
}
